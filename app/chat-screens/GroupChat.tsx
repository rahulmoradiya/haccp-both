...
export default function GroupChatScreen() {
  ...
  const { groupId, groupName, companyCode } = useLocalSearchParams();
  ...
  useEffect(() => {
    if (!currentUser || !companyCode || !groupId) return;

    // Fetch group info
    const fetchGroupInfo = async () => {
      try {
        const groupDoc = await getDoc(doc(db, 'companies', companyCode as string, 'groups', groupId as string));
        if (groupDoc.exists()) {
    ...
    const messagesQuery = query(
      collection(db, 'companies', companyCode as string, 'groups', groupId as string, 'messages'),
    ...
    return unsubscribe;
  }, [currentUser, companyCode, groupId]);
  ...
  const sendMessage = async (attachments?: any[]) => {
    ...
    try {
      // Add message
      const messagesRef = collection(db, 'companies', companyCode as string, 'groups', groupId as string, 'messages');
      ...
      const groupRef = doc(db, 'companies', companyCode as string, 'groups', groupId as string);
  ...
  const showGroupInfo = () => {
    ...
    const memberList = Object.entries(groupInfo.memberNames)
      .map(([id, name]) => name)
      .join(', ');

    Alert.alert(
      groupInfo.name,
      `Description: ${groupInfo.description || 'No description'}\n\nMembers (${groupInfo.members.length}): ${memberList}`,
      [{ text: 'OK' }]
    );
  };
